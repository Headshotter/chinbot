...
# Place a file in your ~/phenny/ folder named, bitly.txt
...
def generateBitLy(phenny, input):
    url = input.group(2)
    if not url:
        if hasattr(phenny, 'last_seen_uri') and input.sender in phenny.bot.last_seen_uri:
            url = phenny.bot.last_seen_uri[input.sender]
        else:
            return phenny.say('No URL provided')

    bitly = short(url)
    for b in bitly:
        displayBitLy(phenny, b[0], b[1])
generateBitLy.commands = ['bitly']
generateBitLy.priority = 'high'


def displayBitLy(phenny, url, shorten):
    if url is None or shorten is None:
        return
    u = getTLD(url)
    shorten = shorten.replace('http:', 'https:')
    phenny.say('%s  -  %s' % (u, shorten))
...
def show_title_auto(phenny, input):
...
def show_title_demand(phenny, input):
    ...
    if not uri:
        ...
        if not hasattr(phenny, 'last_seen_uri'):
            phenny.bot.last_seen_uri = dict()
        if channel in phenny.bot.last_seen_uri:
            uri = phenny.bot.last_seen_uri[channel]
        else:
            return phenny.say('No recent links seen in this channel.')
...
def collect_links(phenny, input):
    link = input.groups()
    channel = input.sender
    link = link[0]
    if not hasattr(phenny, 'last_seen_uri'):
        phenny.bot.last_seen_uri = dict()
    phenny.bot.last_seen_uri[channel] = link
collect_links.rule = '(?iu).*(%s?(http|https)(://\S+)).*' % (EXCLUSION_CHAR)
collect_links.priority = 'low'


def unbitly(phenny, input):
    url = input.group(2)
    if not url:
        if hasattr(phenny, 'last_seen_uri') and input.sender in phenny.bot.last_seen_uri:
            url = phenny.bot.last_seen_uri[input.sender]
    ...
    try:
        ...
        return phenny.say('Failed to grab URL: %s' % (url))
    if u.startswith(('http://', 'https://')):
        phenny.say(u)
    else:
        phenny.say('Failed to obtain final destination.')
